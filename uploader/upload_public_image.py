...
OUTPUT_BUCKET_NAME = 'everybody-public-drive'
    ...
    try:
        ...
        for item in multipart_body.parts:
            ...
            if part.name == 'image':
                ...
                print(f'Elapsed(사진 임시 저장 완료): {finish_tmp_save - start_handle_time}')
                bucket.put_object(
                    # ACL='public-read', # public 권한 필요
                    ContentType='image/png',
                    Key=f'{OUTPUT_OBJECT_PREFIX}{key}',
                    Body=tmp_file.getvalue(),
                )
                tmp_file.close()
                finish_upload_time = time.time()
                print(f'Elapsed(사진 업로드 완료): {finish_upload_time - start_handle_time}')
                keys.append(key)
